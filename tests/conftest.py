...
def script_file_args_path():
    """Return the file path to a script file that logs its own args to the console."""
    yield TEST_SCRIPT_ARGS


@pytest.fixture()
def script_file_hello_path():
    """Return the file path to a script file that logs "hello" to the console."""
    yield TEST_SCRIPT_HELLO


@pytest.fixture()
def node_path():
    """Return the file path to the node installation."""
